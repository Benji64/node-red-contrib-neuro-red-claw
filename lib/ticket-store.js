...
class TicketStore {
  ...
  recent(n = 20, filters = {}) {
    ...
    // v2.10 — copies défensives : ce point de LECTURE externe (dashboard,
    // hub, REST) ne doit jamais exposer la référence vivante qu'un
    // orchestrateur est peut-être encore en train de muter via
    // addToolCall/complete/fail sur ce même ticket "running".
    return list.slice(0, n).map(t => ({ ...t }));
  }
   ...
   */
  get(id) {
    const t = this._cache.find(t => t.id === id);
    return t ? { ...t } : null;
  }
